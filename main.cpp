...
// AABBの衝突判定
bool IsColision(const AABB& aabb01,const AABB& aabb02){
	if ((aabb01.min.x <= aabb02.max.x && aabb01.max.x >= aabb02.min.x) && //X軸
		(aabb01.min.y <= aabb02.max.y && aabb01.max.y >= aabb02.min.y) && //Y軸
		(aabb01.min.z <= aabb02.max.z && aabb01.max.z >= aabb02.min.z)) { //Z軸
		return true;
	}
	return false;
}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		aabb01.min.x = (std::min)(aabb01.min.x, aabb01.max.x); //x
		aabb01.max.x = (std::max)(aabb01.min.x, aabb01.max.x);
		aabb01.min.y = (std::min)(aabb01.min.y, aabb01.max.y); //y
		aabb01.max.y = (std::max)(aabb01.min.y, aabb01.max.y);
		aabb01.min.z = (std::min)(aabb01.min.z, aabb01.max.z); //z
		aabb01.max.z = (std::max)(aabb01.min.z, aabb01.max.z);

		if (IsColision(aabb01, aabb02)) {
			color = RED;
		}
		else {
			color = WHITE;
		...
		DrawAABB(aabb01, worldViewProjectionMatrix, viewportMatrix, color);
		DrawAABB(aabb02, worldViewProjectionMatrix, viewportMatrix, color);

		ImGui::DragFloat3("aabb01.min", &aabb01.min.x, 0.01f);
		ImGui::DragFloat3("aabb01.max", &aabb01.max.x, 0.01f);
		ImGui::DragFloat3("aabb02.min", &aabb02.min.x, 0.01f);
		ImGui::DragFloat3("aabb02.max", &aabb02.max.x, 0.01f);
		ImGui::DragFloat3("Camere", &cameraRotate.x, 0.01f);
